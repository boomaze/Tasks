...
--To get the Suppliers PO expenses
	...
	INTO #TOTALS
	FROM dbo.fnGetSuppliersPOExpenses(@DateStart, @DateEnd) GROUP BY LO_PURCHASE_ORDER_ID,PO_NUMBER--,LO_LOAD_NUMBER
	
	CREATE CLUSTERED INDEX CLU_TOTPO ON #TOTALS (LO_PURCHASE_ORDER_ID)
--To get the POs that are NOT full Billed 
	SELECT * INTO #NOTFULLYBILLED FROM dbo.fnGetPO_NOTFULLYBILLED(@DateStart, @DateEnd)
