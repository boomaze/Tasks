...
BEGIN TRY      
	...
	FROM FP_FREIGHT_FORM AS FRM
		...
		--LEFT JOIN FP_FREIGHT_FORM_VENDOR AS VENDOR 
		--	ON FRM.FP_FREIGHT_FORM_ID = VENDOR.FP_FREIGHT_FORM_ID one to many :(
		INNER JOIN TMS_WHS AS WRHSE
				ON VENDOR.TMS_WHS_ID = WRHSE.TMS_WHS_ID
		INNER JOIN FP_DEFAULT_CONFIG AS CONFIG ON CONFIG.SOURCE_SYSTEM = 	
			  CASE WHEN
			   (NOT EXISTS (SELECT 1 FROM FP_DEFAULT_CONFIG WHERE SOURCE_SYSTEM = WRHSE.SOURCE_SYSTEM))
			  THEN
			   'DEFAULT'
			  ELSE
				WRHSE.SOURCE_SYSTEM
