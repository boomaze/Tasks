		...
		SELECT DISTINCT fff.FP_FREIGHT_FORM_ID, tv.VENDOR_NUMBER as SUPPLIER_NUMBER
			...
			, CASE WHEN WHS.SOURCE_SYSTEM = 'UBS' THEN CAST(fff.EAST_BLENDED_FREIGHT_RATE AS DECIMAL(18,3)) 
					WHEN WHS.TMS_WHS_ID IN (1, 114) THEN CAST(fff.GILROC_BLENDED_FREIGHT_RATE AS DECIMAL(18,3))
					  WHEN WHS.TMS_WHS_ID IN (168, 167) THEN CAST(fff.HARCAR_BLENDED_FREIGHT_RATE AS DECIMAL(18,3))
					   WHEN WHS.TMS_WHS_ID IN (160, 214) THEN CAST(fff.STKSGM_BLENDED_FREIGHT_RATE AS DECIMAL(18,3))
					     WHEN WHS.TMS_WHS_ID IN (178, 186) THEN CAST(fff.ANNSER_BLENDED_FREIGHT_RATE AS DECIMAL(18,3))
			   ELSE 
