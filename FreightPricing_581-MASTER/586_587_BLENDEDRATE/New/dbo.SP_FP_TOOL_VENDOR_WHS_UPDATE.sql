...
/****** Object:  StoredProcedure [dbo].[SP_FP_TOOL_VENDOR_WHS_UPDATE]    Script Date: 9/25/2022 10:35:15 PM ******/
SET ANSI_NULLS ON
GO
...
--  08/09/22 - BK   - RENAMED ORIGINAL CONFIGS TO DEFAULT, ADDED UCS DEFAULT CONFIGS
--  09/12/22 - CM/JM - Fixed bug in direct crossdoc rate calculation (CALC_RATE)
--  09/27/2022 CM   - Added logic for HARCAR, STKSGM, and ANNSER
-- =============================================
ALTER   PROCEDURE [dbo].[SP_FP_TOOL_VENDOR_WHS_UPDATE]
	...
	@USE_EAST_BLENDED_FREIGHT_RATE BIT = 0,
	@AVERAGE_PO_CUBES DECIMAL(18,3) = 0
...
BEGIN TRY      
	...
	DECLARE @PROPOSED_BOOKED_FREIGHT DECIMAL(18,3) = NULL,
			...
			@HARCAR_BLENDED_FREIGHT_RATE AS DECIMAL(10,3) = NULL,
			@STKSGM_BLENDED_FREIGHT_RATE AS DECIMAL(10,3) = NULL,
			@ANNSER_BLENDED_FREIGHT_RATE AS DECIMAL(10,3) = NULL
	...
	BEGIN
		...
		SELECT TOP 1 --@CR_RATE_ID = RT.CR_RATE_ID,
						...
						@RATE = ISNULL((CASE WHEN (RT_PRIMARY_RATE.RATE_UNIT = 'Flat') THEN RT_PRIMARY_RATE.RATE
										  ELSE (CASE WHEN (RT_PRIMARY_RATE.RATE_UNIT = 'Per Mile') THEN (RT_PRIMARY_RATE.RATE * RT_PRIMARY_RATE.MILEAGE)
												END
												)
										  END), 0)
		...
		FROM FP_FREIGHT_FORM_VENDOR AS VENDOR
			...
			WHERE VENDOR.FP_FREIGHT_FORM_VENDOR_ID = @FORM_VENDOR_ID
					...
					--AND (RT.PROTECTION_LEVEL = (CASE WHEN FORM.FP_FF_TEMPERATURE_ID = 3 THEN 'Dry' ELSE RT.PROTECTION_LEVEL END)
					--OR RT_PRIMARY_RATE.PROTECTION_LEVEL = (CASE WHEN FORM.FP_FF_TEMPERATURE_ID = 3 THEN 'Dry' ELSE RT_PRIMARY_RATE.PROTECTION_LEVEL END)
					--	)

					AND (RT.PROTECTION_LEVEL = (CASE WHEN FORM.FP_FF_TEMPERATURE_ID = 3 THEN 'Dry' ELSE RT.PROTECTION_LEVEL END) OR RT.PROTECTION_LEVEL IS NULL)
					AND (RT_PRIMARY_RATE.PROTECTION_LEVEL = (CASE WHEN FORM.FP_FF_TEMPERATURE_ID = 3 THEN 'Dry' ELSE RT_PRIMARY_RATE.PROTECTION_LEVEL END) OR RT_PRIMARY_RATE.PROTECTION_LEVEL IS NULL)
	END
	...
	BEGIN
		--START CALCULATIONS
		SELECT --CALC_RATE
				@CALC_RATE = CASE WHEN @METHOD_ID = 1--BACKHAUL(1)
							 THEN(@AVERAGE_PALLETS_POS * (CASE WHEN DESTINATION.SOURCE_SYSTEM <> 'UCS' THEN FORM.DEFAULT_BACKHAUL_PALLET_RATE ELSE FORM.UCS_BACKHAUL_PALLET_RATE END))
							 WHEN @METHOD_ID = 2 --BACKHAUL CROSSDOCK(2)
							 THEN(@AVERAGE_PALLETS_POS * (CASE WHEN DESTINATION.SOURCE_SYSTEM <> 'UCS' THEN FORM.DEFAULT_CROSSDOCK_PALLET_RATE ELSE FORM.UCS_CROSSDOCK_PALLET_RATE END + CASE WHEN DESTINATION.SOURCE_SYSTEM <> 'UCS' THEN FORM.DEFAULT_BACKHAUL_PALLET_RATE ELSE FORM.UCS_BACKHAUL_PALLET_RATE END) + (@MILES * CONFIG.SHUTTLE_RATE_PER_MILE))
							 WHEN @METHOD_ID = 3 --DIRECT(3)
							 THEN @RATE
							 WHEN @METHOD_ID = 5--DIRECT CROSSDOCK(5)
							 --THEN @RATE
							 THEN (@AVERAGE_PALLETS_POS * (CASE WHEN DESTINATION.SOURCE_SYSTEM <> 'UCS' THEN FORM.DEFAULT_CROSSDOCK_PALLET_RATE ELSE FORM.UCS_CROSSDOCK_PALLET_RATE END) + (@MILES * CONFIG.SHUTTLE_RATE_PER_MILE))
							 END,
				--TOTAL_ALLOWANCE
		...
		SELECT --CALC_ALL_IN_RATE
				@CALC_ALL_IN_RATE = (@CALC_RATE + (@MILES * CASE WHEN DESTINATION.SOURCE_SYSTEM <> 'UCS' THEN FORM.DEFAULT_TOOL_FUEL_SURCHARGE ELSE FORM.UCS_TOOL_FUEL_SURCHARGE END)),
		...
		SELECT  --LANDED_EXPENSE_PER_LB
				@LANDED_EXPENSE_PER_LB = CASE WHEN @METHOD_ID = 1 --BACKHAUL(1)
											THEN CASE	WHEN (@AVERAGE_PO_WEIGHT = 0 OR @AVERAGE_PO_WEIGHT IS NULL) OR (@CALC_ALL_IN_RATE = 0 OR @CALC_ALL_IN_RATE IS NULL) THEN NULL
														ELSE (@CALC_ALL_IN_RATE / @AVERAGE_PO_WEIGHT) + CONFIG.ESTIMATED_LUMPER_EXPENSE + CONFIG.ESTIMATED_ACCESSORIAL_EXPENSE
												 END
										WHEN @METHOD_ID = 2 --BACKHAUL CROSSDOCK(2)
											THEN CASE	WHEN (@AVERAGE_PO_WEIGHT = 0 OR @AVERAGE_PO_WEIGHT IS NULL) OR (@CALC_ALL_IN_RATE = 0 OR @CALC_ALL_IN_RATE IS NULL) THEN NULL 
		...
		SELECT  --@PROPOSED_TOTAL_EXPENSE
				...
				@PROPOSED_FREIGHT_PER_LB = CASE WHEN @BOOKED_FRIEGHT_OVERRIDE = 1 THEN @PROPOSED_BOOKED_FRIEGHT_PER_LB
											--ELSE (@LANDED_EXPENSE_PER_LB * (ISNULL(FORM.TOOL_MARGIN_TARGET, 0) + ISNULL(FORM.TOOL_COOLER_FREEZER_UPCHARGE, 0) + 1) + ISNULL(FORM.TOOL_PFF_PFH_UPCHARGE, 0)) END
												ELSE (@LANDED_EXPENSE_PER_LB * (ISNULL(CASE WHEN DESTINATION.SOURCE_SYSTEM <> 'UCS' THEN FORM.DEFAULT_TOOL_COOLER_FREEZER_UPCHARGE ELSE FORM.UCS_TOOL_COOLER_FREEZER_UPCHARGE END, 0) + 1) + ISNULL(CASE WHEN DESTINATION.SOURCE_SYSTEM <> 'UCS' THEN FORM.DEFAULT_TOOL_MARGIN_TARGET ELSE FORM.UCS_TOOL_MARGIN_TARGET END, 0) + ISNULL(CASE WHEN DESTINATION.SOURCE_SYSTEM <> 'UCS' THEN FORM.DEFAULT_TOOL_PFF_PFH_UPCHARGE ELSE FORM.UCS_TOOL_PFF_PFH_UPCHARGE END, 0)) END
	...
	SET VENDOR.FP_FF_PICKUP_LOCATIONS_ID = @PICKUP_LOCATION_ID, 
		...
		VENDOR.AVERAGE_PO_CUBES = @AVERAGE_PO_CUBES,
		...
		VENDOR.USE_EAST_BLENDED_FREIGHT_RATE = @USE_EAST_BLENDED_FREIGHT_RATE
	FROM FP_FREIGHT_FORM_VENDOR AS VENDOR
	where VENDOR.FP_FREIGHT_FORM_VENDOR_ID = @FORM_VENDOR_ID
	...
	WHERE FORM.FP_FREIGHT_FORM_ID = @FORM_ID
		AND (WHS.SOURCE_SYSTEM='UBS' OR VENDOR.USE_EAST_BLENDED_FREIGHT_RATE = 1) ---logic here to pick records using east 
	...
	INNER JOIN FP_FREIGHT_FORM_VENDOR AS VENDOR
		ON FORM.FP_FREIGHT_FORM_ID = VENDOR.FP_FREIGHT_FORM_ID
		AND (VENDOR.IS_DELETED IS NULL OR VENDOR.IS_DELETED = 0)
		AND VENDOR.USE_EAST_BLENDED_FREIGHT_RATE <> 1    --exclude records using EAST_BLENDED_FREIGHT_RATE
	INNER JOIN TMS_WHS AS WHS 
		ON VENDOR.TMS_WHS_ID = WHS.TMS_WHS_ID
	WHERE FORM.FP_FREIGHT_FORM_ID = @FORM_ID
	...
	WHERE FORM.FP_FREIGHT_FORM_ID = @FORM_ID
		...
		SELECT  --HARCAR_BLENDED_FREIGHT_RATE
			@HARCAR_BLENDED_FREIGHT_RATE = AVG(ISNULL(VENDOR.PROPOSED_FREIGHT_PER_LB, 0))
	FROM FP_FREIGHT_FORM AS FORM
	INNER JOIN FP_FREIGHT_FORM_VENDOR AS VENDOR
		ON FORM.FP_FREIGHT_FORM_ID = VENDOR.FP_FREIGHT_FORM_ID
		AND (VENDOR.IS_DELETED IS NULL OR VENDOR.IS_DELETED = 0)
	...
	INNER JOIN TMS_WHS AS WHS 
		ON VENDOR.TMS_WHS_ID = WHS.TMS_WHS_ID
	WHERE FORM.FP_FREIGHT_FORM_ID = @FORM_ID
	AND WHS.TMS_WHS_ID IN (168, 169)
	GROUP BY FORM.FP_FREIGHT_FORM_ID, FORM.EAST_BLENDED_FREIGHT_RATE;

	UPDATE FORM
	SET HARCAR_BLENDED_FREIGHT_RATE = @HARCAR_BLENDED_FREIGHT_RATE
	FROM FP_FREIGHT_FORM AS FORM
	WHERE FORM.FP_FREIGHT_FORM_ID = @FORM_ID
	
		SELECT  --STKSGM_BLENDED_FREIGHT_RATE
			@STKSGM_BLENDED_FREIGHT_RATE = AVG(ISNULL(VENDOR.PROPOSED_FREIGHT_PER_LB, 0))
	FROM FP_FREIGHT_FORM AS FORM
	INNER JOIN FP_FREIGHT_FORM_VENDOR AS VENDOR
		ON FORM.FP_FREIGHT_FORM_ID = VENDOR.FP_FREIGHT_FORM_ID
		AND (VENDOR.IS_DELETED IS NULL OR VENDOR.IS_DELETED = 0)
	...
	INNER JOIN TMS_WHS AS WHS 
		ON VENDOR.TMS_WHS_ID = WHS.TMS_WHS_ID
	WHERE FORM.FP_FREIGHT_FORM_ID = @FORM_ID
	AND WHS.TMS_WHS_ID IN (160, 214)
	GROUP BY FORM.FP_FREIGHT_FORM_ID, FORM.EAST_BLENDED_FREIGHT_RATE;

	UPDATE FORM
	SET STKSGM_BLENDED_FREIGHT_RATE = @STKSGM_BLENDED_FREIGHT_RATE
	FROM FP_FREIGHT_FORM AS FORM
	WHERE FORM.FP_FREIGHT_FORM_ID = @FORM_ID
	
		SELECT  --ANNSER_BLENDED_FREIGHT_RATE
			@ANNSER_BLENDED_FREIGHT_RATE = AVG(ISNULL(VENDOR.PROPOSED_FREIGHT_PER_LB, 0))
	FROM FP_FREIGHT_FORM AS FORM
	INNER JOIN FP_FREIGHT_FORM_VENDOR AS VENDOR
		ON FORM.FP_FREIGHT_FORM_ID = VENDOR.FP_FREIGHT_FORM_ID
		AND (VENDOR.IS_DELETED IS NULL OR VENDOR.IS_DELETED = 0)
	...
	INNER JOIN TMS_WHS AS WHS 
		ON VENDOR.TMS_WHS_ID = WHS.TMS_WHS_ID
	WHERE FORM.FP_FREIGHT_FORM_ID = @FORM_ID
	AND WHS.TMS_WHS_ID IN (178, 186)
	GROUP BY FORM.FP_FREIGHT_FORM_ID, FORM.EAST_BLENDED_FREIGHT_RATE;

	UPDATE FORM
	SET ANNSER_BLENDED_FREIGHT_RATE = @ANNSER_BLENDED_FREIGHT_RATE
	FROM FP_FREIGHT_FORM AS FORM
	WHERE FORM.FP_FREIGHT_FORM_ID = @FORM_ID
